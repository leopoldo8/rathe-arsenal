...
- **Feature**: uxui-remediation — `.specs/features/uxui-remediation/` — **✅ COMPLETE & VERIFIED; PR #108 open; merged origin/main.** Branch `feat/uxui-remediation` (planning commit `7843a10`). Phase 0 ✅ done: T1 `f694c21` (useFocusTrap), T2 `2d9ecf0` (design-guards scaffold); 1348 tests green. Phase 1 ✅ done (1a: T3 b37b2f1, T4 47e95d8, T7 e61c48e, T8 3a41c9c; 1b: T5 470df8c, T6 4114b63, T9 a8feea9, T10 e8447c7) — all 3 P0 + P1 a11y/bans cleared, 1419 tests green, regression guards (focus/side-stripe/gradient-text) live. Phase 2 ✅ done (T11 aaf8352 ReadinessHero mounted in canvas + sidebar dup removed; T12 07e73e0 dead code removed) — build gate green, 1405 tests. Phase 3 ✅ done (T13 c40a18e, T14 6c58a15, T15 f8eb6f2, T16 03e0169, T17 12d915c) — build gate green, 1419 tests. Phase 4 ✅ done (T18 1ad6235, T19 6dd9fe3, T20 2fdd43b, T21 cf4b561) — build gate green, 1438 tests. ALL behavioral tasks T1–T21 complete. Phase 5 ✅ done (T22 3c80222, T23 30cb293, T24 86c758d) — unit/build gate green, 1438 tests; visual snapshot regen deferred to CI (doc in docs/design/v1/visual-regression.md). ALL 24 tasks complete. Verifier (Opus, independent) returned **PASS** — gate 1438/0, sensor 3/3 mutants killed, 5 non-blocking spec-precision gaps (CSS-value ACs not pinned by an assertion) + 3 CI-deferred visual ACs; report `.specs/features/uxui-remediation/validation.md`, lesson L-003 (candidate). 5 spec-precision gaps closed (9b959ab, +10 value-pinning guard assertions). **✅ FEATURE COMPLETE, VERIFIED & VISUALLY RE-BASELINED.** ~31 commits on `feat/uxui-remediation`. Orchestrator re-ran the build gate itself: typecheck clean, lint clean, **1448 unit passed / 1 pre-existing skip / 0 failed**. Verifier PASS (3/3 mutants killed). VISUAL: owner brought up the local stack (api+web+OrbStack Postgres), so the re-baseline was done locally (no longer CI-deferred): found + fixed a pre-existing fixture bug — `resolveDeckUrl` matched the `/decks/new` CTA so deck-detail captured the add-deck page; excluded it (commit dc04225). **All 21 visual surfaces pass**; onboarding now captures the wizard, deck-detail the populated deck with the ReadinessHero focal point (D1 verified live). **PR OPENED: https://github.com/leopoldo8/rathe-arsenal/pull/108** (base `main`). Branch pushed to origin; CI will run (visual baselines committed; CI has a Postgres service per the i18n precedent). Remaining: owner review/merge of #108. Lesson L-003 candidate in lessons store. NOTE: `apps/web/test-results/` is NOT gitignored (pre-existing) — playwright run artifacts left untracked, not committed. ENV: no local Postgres/Docker (confirmed — nothing on :5432/:3000/:5173, no pg binaries), so visual snapshot regeneration is DEFERRED TO CI (mirrors i18n precedent); Phase 5 does fixture CODE only. Changed surfaces needing `playwright --update-snapshots` in CI: wordmark/TopBar, Path C banner, auth error, TestDeckResult, deck-detail (ReadinessHero layout), DeckCard SVG colors, lightbox shimmer, home + deck-detail skeletons, touch-target sizing, add-cards numerals. After Phase 5: Opus Verifier over the unit/behavioral surface. Worker-per-phase, Sonnet workers, Opus Verifier at the end. NOTE: quick gate (vitest) does not typecheck — phase workers must end on a BUILD gate to catch latent TS errors.
...
- **Merged from origin/main into this branch** (to clear conflicts before #108 merges): #106 swap-copies-grouping (AD-005, frontend-only `× N` grouping on Swaps + deck-detail) and #107 residual i18n. Resolved: kept dead-component deletions (readiness-header etc. still 0 imports on main; #107 only localized a dead file); adopted main's `pitchLabelKeyFor`/`library.pitch*Label` for the add-cards pitch aria-label and dropped the duplicate `decks.pitchColor*` keys. Re-ran the full gate after merge.
- **Prior completed features** (on main): swap-copies-grouping `.specs/features/swap-copies-grouping/` (PASS) and i18n `.specs/features/i18n/` (PASS).
- **Blockers**: none.
