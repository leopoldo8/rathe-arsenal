...
export class SbraubleScraperService {
  ...
  private parsePriceAndStock(rawPrice: string, rawStock: string): { priceCents: number | null; quantity: number } {
    ...
    const priceCents = this.parsePriceCents(rawPrice);
    const quantity = this.parseQuantity(rawStock);
   ...
   */
  private parsePriceCents(rawPrice: string): number {
    // Strip leading "R$ " and optional whitespace, then strip thousands separators (dots),
    // then replace the decimal comma with a dot for parseFloat.
   ...
   */
  private parseQuantity(rawStock: string): number {
