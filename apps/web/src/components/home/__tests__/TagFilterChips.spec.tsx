...
describe('TagFilterChips', () => {
  ...
  it('renders a chip for each available tag', () => {
    ...
    expect(screen.getByRole('button', { name: /filtrar por tag: league/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /filtrar por tag: casual/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /filtrar por tag: competitive/i })).toBeInTheDocument();
  });

  it('inactive chips have aria-pressed=false', () => {
    renderChips(['league'], []);
    const chip = screen.getByRole('button', { name: /filtrar por tag: league/i });
  ...
  it('clicking a second chip ORs it with the first (adds to the filter set)', () => {
    ...
    const chip = screen.getByRole('button', { name: /filtrar por tag: casual/i });
    fireEvent.click(chip);
  ...
  it('does not show Clear button when no tag is active', () => {
    renderChips(['league'], []);
    expect(screen.queryByRole('button', { name: /limpar todos os filtros de tag/i })).not.toBeInTheDocument();
  });

  it('shows Clear button when at least one tag is active', () => {
    renderChips(['league'], ['league']);
    expect(screen.getByRole('button', { name: /limpar todos os filtros de tag/i })).toBeInTheDocument();
  });

  it('clicking Clear calls onFilterChange with []', () => {
    const onFilterChange = vi.fn();
    renderChips(['league', 'casual'], ['league', 'casual'], onFilterChange);
    const clearBtn = screen.getByRole('button', { name: /limpar todos os filtros de tag/i });
  ...
  it('renders the component as a group with an accessible label', () => {
    renderChips(['league']);
    expect(screen.getByRole('group', { name: /filtrar por tag/i })).toBeInTheDocument();
  });
  ...
  describe('URL filter persistence via onFilterChange', () => {
    it('preserving other search params is the caller responsibility (tested in home.tsx integration)', () => {
      ...
      fireEvent.click(screen.getByRole('button', { name: /filtrar por tag: bar/i }));
