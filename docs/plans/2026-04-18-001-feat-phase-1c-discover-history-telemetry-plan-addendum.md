...
1. **Source = `isTournament:true`, formats = CC + Blitz + Silver Age + Living Legend.** Discover ingests Algolia `public_decks` filtered by `isTournament:true` AND `format ∈ {Classic Constructed, Blitz, Silver Age, Living Legend}`, sorted by `updatedAt DESC`. The filter expression is wired through an env var (`DISCOVER_ALGOLIA_FILTER`, default `isTournament:true AND (format:"Classic Constructed" OR format:"Blitz" OR format:"Silver Age" OR format:"Living Legend")`) so the operator can widen to `hasResults:true`, default-sort, or different format scopes without a code change. Engine-side: `computeEffectiveReadiness` does not enforce format legality (verified 2026-04-19) — readiness is computed against the card list regardless of format, so SA/LL preview readiness produces meaningful numbers without engine changes. The `bannedFormats` / `legalFormats` catalog fields are not consumed in the readiness path; Phase 2 may add a format-legality lint as a separate concern.
...
6. **Two-step ingestion is now the default path, not a contingency.** The parent plan's Unit 1 had a contingency clause for "thin trending response shape requires per-deck `getDeck` follow-ups". With Algolia, every ingestion run does:
   ...
   3. Apply R14 quality filter (hero in catalog, mainboardCount ≥ 60 for CC/SA/LL or ≥ 40 for Blitz, format ∈ {Classic Constructed, Blitz, Silver Age, Living Legend}) — `mainboardCount` only knowable AFTER step 2. Note: Blitz uses 40-card minimum (per the FaB rules), so the `mainboardCount` floor is format-aware. CC, Silver Age, and Living Legend all use the 60-card floor.
...
| Unit 3 → Filter DTO | Drop `archetype?` from `discover-list.query.ts`. Drop the `?archetype=Aggro` test scenario. **Expand `format` enum** from `'CC' \| 'Blitz'` to `'Classic Constructed' \| 'Blitz' \| 'Silver Age' \| 'Living Legend'` (use the full Algolia format strings as the canonical values to avoid lossy translation). Frontend filter dropdown shows 4 options instead of 2 |
